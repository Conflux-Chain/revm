...
use crate::{instructions::*, primitives::Spec};
...
macro_rules! opcodes {
    ($($val:literal => $name:ident => $f:expr => $($modifier:ident $(( $($modifier_arg:expr),* ))?),*);* $(;)?) => {
        ...
        pub const fn instruction<SPEC: Spec>(opcode: u8) -> Instruction {
            match opcode {
                $($name => $f,)*
                _ => control::unknown,
            }
...
opcodes! {
    ...
    0x0A => EXP        => arithmetic::exp::<SPEC> => stack_io(2, 1);
    ...
    0x1B => SHL    => bitwise::shl::<SPEC> => stack_io(2, 1);
    0x1C => SHR    => bitwise::shr::<SPEC> => stack_io(2, 1);
    0x1D => SAR    => bitwise::sar::<SPEC> => stack_io(2, 1);
    // 0x1E
    // 0x1F
    ...
    0x31 => BALANCE      => host::balance::<SPEC> => stack_io(1, 1);
    ...
    0x3B => EXTCODESIZE    => host::extcodesize::<SPEC>      => stack_io(1, 1), not_eof;
    0x3C => EXTCODECOPY    => host::extcodecopy::<SPEC>      => stack_io(4, 0), not_eof;
    0x3D => RETURNDATASIZE => system::returndatasize::<SPEC> => stack_io(0, 1);
    0x3E => RETURNDATACOPY => system::returndatacopy::<SPEC> => stack_io(3, 0);
    0x3F => EXTCODEHASH    => host::extcodehash::<SPEC>      => stack_io(1, 1), not_eof;
    0x40 => BLOCKHASH      => host::blockhash::<SPEC>          => stack_io(1, 1);
    ...
    0x44 => DIFFICULTY     => host_env::difficulty::<SPEC>   => stack_io(0, 1);
    0x45 => GASLIMIT       => host_env::gaslimit                => stack_io(0, 1);
    0x46 => CHAINID        => host_env::chainid::<SPEC>      => stack_io(0, 1);
    0x47 => SELFBALANCE    => host::selfbalance::<SPEC>      => stack_io(0, 1);
    0x48 => BASEFEE        => host_env::basefee::<SPEC>      => stack_io(0, 1);
    0x49 => BLOBHASH       => host_env::blob_hash::<SPEC>    => stack_io(1, 1);
    0x4A => BLOBBASEFEE    => host_env::blob_basefee::<SPEC> => stack_io(0, 1);
    // 0x4B
    // 0x4C
    // 0x4D
    // 0x4E
    ...
    0x54 => SLOAD    => host::sload::<SPEC>   => stack_io(1, 1);
    0x55 => SSTORE   => host::sstore::<SPEC>  => stack_io(2, 0);
    ...
    0x5C => TLOAD    => host::tload::<SPEC>   => stack_io(1, 1);
    0x5D => TSTORE   => host::tstore::<SPEC>  => stack_io(2, 0);
    0x5E => MCOPY    => memory::mcopy::<SPEC> => stack_io(3, 0);

    0x5F => PUSH0  => stack::push0::<SPEC> => stack_io(0, 1);
    0x60 => PUSH1  => stack::push::<1>     => stack_io(0, 1), immediate_size(1);
    0x61 => PUSH2  => stack::push::<2>     => stack_io(0, 1), immediate_size(2);
    0x62 => PUSH3  => stack::push::<3>     => stack_io(0, 1), immediate_size(3);
    0x63 => PUSH4  => stack::push::<4>     => stack_io(0, 1), immediate_size(4);
    0x64 => PUSH5  => stack::push::<5>     => stack_io(0, 1), immediate_size(5);
    0x65 => PUSH6  => stack::push::<6>     => stack_io(0, 1), immediate_size(6);
    0x66 => PUSH7  => stack::push::<7>     => stack_io(0, 1), immediate_size(7);
    0x67 => PUSH8  => stack::push::<8>     => stack_io(0, 1), immediate_size(8);
    0x68 => PUSH9  => stack::push::<9>     => stack_io(0, 1), immediate_size(9);
    0x69 => PUSH10 => stack::push::<10>    => stack_io(0, 1), immediate_size(10);
    0x6A => PUSH11 => stack::push::<11>    => stack_io(0, 1), immediate_size(11);
    0x6B => PUSH12 => stack::push::<12>    => stack_io(0, 1), immediate_size(12);
    0x6C => PUSH13 => stack::push::<13>    => stack_io(0, 1), immediate_size(13);
    0x6D => PUSH14 => stack::push::<14>    => stack_io(0, 1), immediate_size(14);
    0x6E => PUSH15 => stack::push::<15>    => stack_io(0, 1), immediate_size(15);
    0x6F => PUSH16 => stack::push::<16>    => stack_io(0, 1), immediate_size(16);
    0x70 => PUSH17 => stack::push::<17>    => stack_io(0, 1), immediate_size(17);
    0x71 => PUSH18 => stack::push::<18>    => stack_io(0, 1), immediate_size(18);
    0x72 => PUSH19 => stack::push::<19>    => stack_io(0, 1), immediate_size(19);
    0x73 => PUSH20 => stack::push::<20>    => stack_io(0, 1), immediate_size(20);
    0x74 => PUSH21 => stack::push::<21>    => stack_io(0, 1), immediate_size(21);
    0x75 => PUSH22 => stack::push::<22>    => stack_io(0, 1), immediate_size(22);
    0x76 => PUSH23 => stack::push::<23>    => stack_io(0, 1), immediate_size(23);
    0x77 => PUSH24 => stack::push::<24>    => stack_io(0, 1), immediate_size(24);
    0x78 => PUSH25 => stack::push::<25>    => stack_io(0, 1), immediate_size(25);
    0x79 => PUSH26 => stack::push::<26>    => stack_io(0, 1), immediate_size(26);
    0x7A => PUSH27 => stack::push::<27>    => stack_io(0, 1), immediate_size(27);
    0x7B => PUSH28 => stack::push::<28>    => stack_io(0, 1), immediate_size(28);
    0x7C => PUSH29 => stack::push::<29>    => stack_io(0, 1), immediate_size(29);
    0x7D => PUSH30 => stack::push::<30>    => stack_io(0, 1), immediate_size(30);
    0x7E => PUSH31 => stack::push::<31>    => stack_io(0, 1), immediate_size(31);
    0x7F => PUSH32 => stack::push::<32>    => stack_io(0, 1), immediate_size(32);

    0x80 => DUP1  => stack::dup::<1>  => stack_io(1, 2);
    0x81 => DUP2  => stack::dup::<2>  => stack_io(2, 3);
    0x82 => DUP3  => stack::dup::<3>  => stack_io(3, 4);
    0x83 => DUP4  => stack::dup::<4>  => stack_io(4, 5);
    0x84 => DUP5  => stack::dup::<5>  => stack_io(5, 6);
    0x85 => DUP6  => stack::dup::<6>  => stack_io(6, 7);
    0x86 => DUP7  => stack::dup::<7>  => stack_io(7, 8);
    0x87 => DUP8  => stack::dup::<8>  => stack_io(8, 9);
    0x88 => DUP9  => stack::dup::<9>  => stack_io(9, 10);
    0x89 => DUP10 => stack::dup::<10> => stack_io(10, 11);
    0x8A => DUP11 => stack::dup::<11> => stack_io(11, 12);
    0x8B => DUP12 => stack::dup::<12> => stack_io(12, 13);
    0x8C => DUP13 => stack::dup::<13> => stack_io(13, 14);
    0x8D => DUP14 => stack::dup::<14> => stack_io(14, 15);
    0x8E => DUP15 => stack::dup::<15> => stack_io(15, 16);
    0x8F => DUP16 => stack::dup::<16> => stack_io(16, 17);

    0x90 => SWAP1  => stack::swap::<1>  => stack_io(2, 2);
    0x91 => SWAP2  => stack::swap::<2>  => stack_io(3, 3);
    0x92 => SWAP3  => stack::swap::<3>  => stack_io(4, 4);
    0x93 => SWAP4  => stack::swap::<4>  => stack_io(5, 5);
    0x94 => SWAP5  => stack::swap::<5>  => stack_io(6, 6);
    0x95 => SWAP6  => stack::swap::<6>  => stack_io(7, 7);
    0x96 => SWAP7  => stack::swap::<7>  => stack_io(8, 8);
    0x97 => SWAP8  => stack::swap::<8>  => stack_io(9, 9);
    0x98 => SWAP9  => stack::swap::<9>  => stack_io(10, 10);
    0x99 => SWAP10 => stack::swap::<10> => stack_io(11, 11);
    0x9A => SWAP11 => stack::swap::<11> => stack_io(12, 12);
    0x9B => SWAP12 => stack::swap::<12> => stack_io(13, 13);
    0x9C => SWAP13 => stack::swap::<13> => stack_io(14, 14);
    0x9D => SWAP14 => stack::swap::<14> => stack_io(15, 15);
    0x9E => SWAP15 => stack::swap::<15> => stack_io(16, 16);
    0x9F => SWAP16 => stack::swap::<16> => stack_io(17, 17);

    0xA0 => LOG0 => host::log::<0> => stack_io(2, 0);
    0xA1 => LOG1 => host::log::<1> => stack_io(3, 0);
    0xA2 => LOG2 => host::log::<2> => stack_io(4, 0);
    0xA3 => LOG3 => host::log::<3> => stack_io(5, 0);
    0xA4 => LOG4 => host::log::<4> => stack_io(6, 0);
    // 0xA5
    // 0xA6
    // 0xA7
    // 0xA8
    ...
    // 0xEF
    0xF0 => CREATE       => contract::create::<false, SPEC> => stack_io(3, 1), not_eof;
    0xF1 => CALL         => contract::call::<SPEC>          => stack_io(7, 1), not_eof;
    0xF2 => CALLCODE     => contract::call_code::<SPEC>     => stack_io(7, 1), not_eof;
    0xF3 => RETURN       => control::ret                       => stack_io(2, 0), terminating;
    0xF4 => DELEGATECALL => contract::delegate_call::<SPEC> => stack_io(6, 1), not_eof;
    0xF5 => CREATE2      => contract::create::<true, SPEC>  => stack_io(4, 1), not_eof;
    // 0xF6
    0xF7 => RETURNDATALOAD  => system::returndataload                => stack_io(1, 1);
    0xF8 => EXTCALL         => contract::extcall::<SPEC>          => stack_io(4, 1);
    0xF9 => EXTDELEGATECALL => contract::extdelegatecall::<SPEC>  => stack_io(3, 1);
    0xFA => STATICCALL      => contract::static_call::<SPEC>      => stack_io(6, 1), not_eof;
    0xFB => EXTSTATICCALL   => contract::extstaticcall               => stack_io(3, 1);
    // 0xFC
    0xFD => REVERT       => control::revert::<SPEC>    => stack_io(2, 0), terminating;
    0xFE => INVALID      => control::invalid              => stack_io(0, 0), terminating;
    0xFF => SELFDESTRUCT => host::selfdestruct::<SPEC> => stack_io(1, 0), not_eof, terminating;
}
