...
use quote::quote;
use syn::{parse_macro_input, GenericParam, ItemFn, parse_quote};
...
pub fn use_dyn_host(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    sig.generics.params = sig.generics.params.clone().into_iter().filter(|param| {
    ...
    if !sig.generics.params.iter().any(|param| matches!(param, GenericParam::Lifetime(_))) {
        ...
        if let syn::FnArg::Typed(pat_type) = input {
            if let syn::Type::Reference(type_reference) = &mut *pat_type.ty {
                if let syn::Type::Path(type_path) = &mut *type_reference.elem {
                    if type_path.path.segments.last().map_or(false, |seg| seg.ident == "H") {
